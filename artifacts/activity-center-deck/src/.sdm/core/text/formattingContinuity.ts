...
import { RunStyleSchema, type RunStyle } from '../schema';
import {
...
function defaultStyleOf(node: ProseMirrorNode): RunStyle | undefined {
  const value: unknown = node.attrs.defaultRunStyle;

  return Value.Check(RunStyleSchema, value)
    ? canonicalizeRunStyle(value)
    : undefined;
}
  ...
  props: {
    handleTextInput(view, from, to, text) {
      ...
      const marks =
        ...
        (style === undefined ? [] : marksFromRunStyle(style));
      if (marks.length === 0) {
        return false;
      }
      view.dispatch(
