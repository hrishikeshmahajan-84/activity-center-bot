...
import { runStyleFromMarks } from './pmDoc';
...
export function paragraphMarkers(
  paragraphs: ReadonlyArray<Pick<Paragraph, 'bullet' | 'level'>>,
): Array<string> {
...
function paragraphMetadata(doc: ProseMirrorNode) {
  const paragraphs: Array<Pick<Paragraph, 'bullet' | 'level'>> = [];
  doc.forEach((node) => {
    paragraphs.push(paragraphMetadataFromNode(node));
  });
...
export function createParagraphMarkerPlugin(theme?: Theme): Plugin {
  return new Plugin({
    props: {
      decorations(state) {
        const markers = paragraphMarkers(paragraphMetadata(state.doc));
          ...
          decorations.push(
            Decoration.widget(
              ...
              () => {
                ...
                Object.assign(
                  span.style,
                  paragraphMarkerCss(paragraphForMarker(node), theme),
                );
