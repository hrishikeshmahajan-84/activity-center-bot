...
function layoutText(body: TextBody, frame: Frame): TextLayout {
  ...
  for (const paragraph of body.paragraphs) {
    cursorY += (paragraph.spaceBeforePt ?? 0) * 2;
    const paragraphLines = layoutParagraph(paragraph, availableWidth);
    ...
    cursorY += (paragraph.spaceAfterPt ?? 0) * 2;
  }
...
function defaultLineHeight(paragraph: Paragraph): number {
  ...
  return (largest || DEFAULT_SIZE_PT) * 2 * (paragraph.lineHeight ?? 1.2);
}
